...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for i in range(3):
            l = reader.next()  # why it should skip these three rows
            # print l

        for line in reader:
            # YOUR CODE HERE
            line_data = template_field
            line_data['synonym'] = line['synonym'].strip()
            line_data['name'] = line_data['name'].strip()
            line_data['classification']['kindom'] = line['kingdom_label'].strip()
            line_data['classification']['family'] = line['family_label'].strip()
            line_data['classification']['order'] = line['order_label'].strip()
            line_data['classification']['phylum'] = line['phylum_label'].strip()
            line_data['classification']['genus'] = line['genus_label'].strip()
            line_data['classification']['class'] = line['class_label'].strip()
            line_data['uri'] = line['URI'].strip()
            line_data['label'] = line['rdf-schema#label'].strip()
            if line_data['label'].index('(')>=0:
                label = line_data['label'].split('(')
                line_data = label[0]
            line_data['description'] = line['rdf-schema#comment'].strip()
