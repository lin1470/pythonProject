...
def process_all(datadir):
    ...
    return files
def to_int(string):
    thousand,unit = string.split(',')
    return int(thousand)*1000+int(unit)

def process_file(f='FL-ATL.html'):
    """
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for tr in trs[1:-1]:
            tds = tr.find_all('td')  # 这样子返回就是一个列表了。
            if tds[1].text == 'TOTAL':
                continue
            else:
                # print tds[0].text, " ", tds[1].text, " ", to_int(tds[2].text), " ", to_int(tds[3].text), " ", tds[4].text
                info['year'] = int(tds[0].text)
                info['month'] = int(tds[1].text)
                flights ={}
                flights['domestic'] = to_int(tds[2].text)
                flights['international'] = to_int(tds[3].text)
